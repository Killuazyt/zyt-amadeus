...
P7H adds an immutable request-scoped capability snapshot and auditable, version-bound companion cues. Text, completed voice transcripts, attachments, and explicitly shared screen/window/camera frames each receive exact perception boundaries that retries cannot expand. Follow-up cues are proposed by the existing memory-extraction response but remain unusable until the user edits and confirms them. Contextual follow-ups are off by default; an enabled cue can produce one privacy-safe generic bubble, and clicking it expands the frozen text locally without a provider request. Static Kurisu knowledge can never authorize a follow-up.

P7I adds provider-free, one-shot exact reminders and scheduled follow-ups. Explicit Chinese or English reminder commands are parsed before model availability checks and become editable local drafts; an unconfirmed draft never fires. Confirmed work is scheduled in-process with one nearest-deadline timer plus calibration polling. Exact reminders use privacy-safe tray notifications, scheduled follow-ups use a local frozen companion line when interaction conditions are safe, and missed work is recovered after resume or the next launch without claiming background delivery while the application is closed.

## P7H validation status

P7H local acceptance completed on 2026-08-18 using only fake credentials and mock transports. The final complete desktop suite passed `985` tests with `3` environment-specific skips in `132.03s`; Ruff, format verification, `compileall`, `pip check`, diff verification, and both source and existing-artifact privacy scans passed. The final source candidate scan covered 246 files with zero violations. The existing build-output scan covered 686 files and 487 archive members with zero violations; P7H did not create a package, onedir, installer, or release artifact.

The new cue-management, proactive-follow-up, and chat-source surfaces also passed a real Windows display smoke at DPR `1.25`: logical sizes were `1500 x 1080`, `900 x 760`, and `520 x 680`, with no clipping or private cue text in the desktop bubble. No provider or credential access occurred. Test-only Desktop CI run `32155341664` precisely matched implementation SHA `11e6b15ac66429f0f16f89b72b844306254850d2` and passed with `987 passed, 1 skipped`; all package/onedir/installer steps were skipped and no artifacts were produced. Final documentation-SHA evidence is recorded in [desktop/P7H-ACCEPTANCE.md](desktop/P7H-ACCEPTANCE.md). P8 remains unstarted and separate from this work.
