...
Runtime data is stored under `%LOCALAPPDATA%\Amadeus`; the application keeps `data\amadeus.sqlite3`, managed attachments, settings, imported resources, local persona material, model cache, backups, and redacted logs in separate validated regions. Settings schema v8 and SQLite schema v5 contain only non-secret state and credential references; exports and backups never read Windows Credential Manager. Text, multimodal, ASR, and TTS credentials remain in Windows Credential Manager. Deterministic local simulation is available only through the explicit `--mock-chat` development flag or test injection.
